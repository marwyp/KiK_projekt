...
class ArithmeticCoder:
    ...
    def __init__(self):
        ...
        self.tag = Decimal((self.left_boundary + self.right_boundary)/2)
    ...
    def coder_static(self, text, visualize=False, custom_prob={}):
        ...
        if len(custom_prob.keys()) == 0:
            ...
            for letter in self.freq_table:
                self.probability_table[letter] = Decimal(self.freq_table[letter] / self.all_char_counter).normalize()
            decimal.getcontext().prec = self.COMP_PREC
        else:
